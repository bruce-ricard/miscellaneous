...
\frame{
  ...
  \note {In integers, the square function is increasing. We can use
    binary search to find a square root}
  \note {In modular arithmetic,
    you can see that the results appear to be quite random. That's how
    we make a good one way function}
  \note {This function is very easy to calculate, but it is also very
    hard to find all numbers x such that f(x) = 81, by opposition to
...
\note {we can't trust our human eyes to decide if the data is securely
encrypted. It's not because you can't understand something, that
nobody can. Humility.}

echo 'mysafepassword123!' | base64 -
bXlzYWZlcGFzc3dvcmQxMjMhCg==

echo 'mysafepassword123!' | sha1sum
8df488f983bdea4d1e071db7f5e3c7189b9a5a4a
...
\frame{
  ...
  \item Sign RSA public keys / owned by the correct entity  \pause : certificates
  \end{itemize}
}
...
\frame{
  ...
  \note {PKI is the term used to describe a system using certificates and cryptography for its members to communicate safely on the Internet.}
...
\frame{
  ...
  over the wire with their common key, using a symetric algorithm like AES.
}
